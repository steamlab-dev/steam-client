/**
 * Builds the generated Steam language bundles consumed by the library.
 *
 * It scans the generated Steam protobuf definition files, collects exported
 * enums, service classes, and proto payload types, groups them by namespace,
 * and writes consolidated bundle files under `src/common/steam-language`.
 *
 * The generated output includes namespace-specific bundles for enums,
 * services, and proto payloads, plus a root `index.ts` that re-exports the
 * generated language surface used by the rest of the project.
 */
...
interface CategorizedExports {
  ...
  services: ExportInfo[];
  protos: ExportInfo[];
  protoInterfaces: ExportInfo[];
  protoTypeAliases: ExportInfo[];
}
 ...
 */
const getNamespace = (filePath: string): string | null => {
...
const groupByNamespace = (exports: ExportInfo[]) => {
  ...
  exports.forEach((exp) => {
    const ns = getNamespace(exp.filePath);
...
function categorizeExports(allExports: ExportInfo[]): CategorizedExports {
  const enums = allExports.filter((entry) => entry.type === "enum");
  const services = allExports.filter((entry) => entry.type === "class");
  const protoInterfaces = allExports.filter((entry) => entry.type === "interface");
  const protoTypeAliases = allExports.filter(
    (entry) => entry.type === "type" && entry.name.startsWith("CMsg"),
  );

  return {
    enums,
    services,
    protos: [...protoInterfaces, ...protoTypeAliases],
    protoInterfaces,
    protoTypeAliases,
  };
}

function renderEnumBundle(exports: ExportInfo[], namespace: string): string {
  const imports = createImports(exports);
  const uniqueNames = [...new Set(exports.map((entry) => entry.name))].sort();

  return [
    ...config.header,
    ...imports,
    "",
    `export const ${namespace}Enums = {`,
    ...uniqueNames.map((name) => `  ${name}: ${name},`),
    "} as const;",
  ].join("\n");
}

function renderInterfaceBundle(exports: ExportInfo[], interfaceName: string): string {
  ...
  return [
    ...
    ...sortedExports.map((entry) => `  ${entry.name}: ${entry.name};`),
    "}",
  ].join("\n");
}

function renderIndexContent(
  enumNamespaces: string[],
  serviceNamespaces: string[],
  protoNamespaces: string[],
): string {
  return [
    ...config.header,
    "",
...
function queueBundleWrite(
  ...
  writePromises.push(
    writeFileAsync(filePath, content).then(() => console.log(`   - Generated ${logName}`)),
  );
}
...
export default async function buildSteamLanguage(): Promise<void> {
  try {
    ...
    const { enums, services, protos, protoInterfaces, protoTypeAliases } =
      categorizeExports(allExports);
    console.log(
    ...
    for (const [ns, exports] of groupedEnums.entries()) {
      ...
      queueBundleWrite(
        writePromises,
        filePath,
        renderEnumBundle(exports, ns),
        config.templates.enumFile(ns),
      );
    }
    ...
    for (const [ns, exports] of groupedServices.entries()) {
      ...
      queueBundleWrite(
        writePromises,
        filePath,
        renderInterfaceBundle(exports, interfaceName),
        config.templates.serviceFile(ns),
      );
    }
    ...
    await writeFileAsync(
      ...
      renderIndexContent(enumNamespaces, serviceNamespaces, protoNamespaces),
    );
